// Prácticas guiadas de Zerena.
// Los guiones se derivan EXACTAMENTE del contenido que ya existe en la app
// (/para-mi/respira, /para-mi/presente, /para-mi/descansar).
// No se añade consejo clínico nuevo. La IA solo pone la voz a este texto fijo.
...
export type Practice = {
  ...
  duration: string;
  summary: string;
  /** Ruta a la práctica escrita equivalente (versión silenciosa). */
  silentPath: string;
  silentLabel: string;
  /** Guion leído en voz alta = transcripción visible. */
  script: string[];
};
...
/** URL estable del audio de una práctica (cacheable por el service worker). */
