...
const MESSAGES: Record<number, string> = {
  ...
  429: "Muchas personas pidiendo calma a la vez. Intenta de nuevo en un momento.",
...
export function PracticePlayer({ practice }: { practice: Practice }) {
  ...
  async function ensureAudio(): Promise<HTMLAudioElement | null> {
    ...
    setError(null);
    try {
      const res = await fetch(practiceAudioUrl(practice.id));
      if (!res.ok) {
        setStatus("error");
        setError(
          MESSAGES[res.status] ??
      ...
      {status === "error" && error && (
        <div className="mt-4 rounded-2xl border border-border bg-secondary/60 p-4 text-sm text-foreground">
          <p className="flex items-start gap-2">
            {offline && <WifiOff size={16} className="mt-0.5 shrink-0 text-sage-deep" aria-hidden />}
            <span>{error}</span>
          </p>
